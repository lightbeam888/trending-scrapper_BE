    ...
    try {
        ...
        if (messageText) {
            ...
            result = lines.map((line) => {
                ...
                const marketCap = $line.html()?.split('|')[2]?.trim().replace(/<i\b[^>]*>(.*?)<\/i>/gi, '').replace(/<[^>]*>/g, '').trim().replace(/MC/g, '').trim();
                const address = $line('a').last().attr("href")?.toString().slice(28, 82);
                const priceChange = $line('a').length === 2 ? $line('a').eq(1).text().trim().replace(/[\s+]+/g, '') : $line('a').first().text().trim().replace(/[\s+]+/g, '');

                if(priceChange === "BASETRENDING") {
                    ethCompleted = true;
                }

                if(priceChange === "BSCTRENDING") {
                    baseCompleted = true;
                }

                if(!ethCompleted) return;
                if(!baseCompleted) return;

                if(tokenName.length > 0 && priceChange !== "BscTRENDING" && isPercentage(priceChange)) {
