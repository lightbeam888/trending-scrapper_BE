    ...
    try {
        ...
        if (messageText) {
            ...
            result = lines.map((line) => {
                ...
                const marketCap = $line.html()?.split('|')[1]?.replace(/<\/?[^>]+(>|$)/g, "").trim();
                const address = $line('a').first().attr("href")?.toString().slice(36);
                //const priceChange = $line('a').last().text().trim();

                if(tokenName.length > 0 && tokenName !== "Earn SOL from Finder Trending" && marketCap !== undefined) {
                    return {tokenName, marketCap: convertMarketCap(marketCap), priceChange: 0, address};
                }
