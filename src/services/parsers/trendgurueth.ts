    ...
    try {
        ...
        if (messageText) {
            ...
            result = lines.map((line) => {
                ...
                const address = $line('a').last().attr("href")?.toString().slice(42);
                const priceChange = $line('a').last().text().trim();

                if(tokenName.length > 0 && tokenName !== "Trending Available!" && isPercentage(priceChange)) {
                    return {tokenName, marketCap: convertMarketCap(marketCap), priceChange: parseFloat(priceChange.replace(/[^\d.-]/g, '')), address};
                }
